...
export default function EditPost() {
  ...
  useEffect(() => {
    const fetchPost = async () => {
      try {
        ...
        if (post.user_id !== user?.id && post.user?.id !== user?.id) {
        ...
        setTitle(post.title || '');
        setWorkingDomain(post.working_domain || '');
        setRequiredExpertise(post.required_expertise || '');
        setShortExplanation(post.short_explanation || '');
        setConfidentialityLevel(post.confidentiality_level || 'PUBLIC_PITCH');
        setCollaborationType(post.collaboration_type || 'ADVISOR');
        setProjectStage(post.project_stage || 'IDEA');
        setCity(post.city || '');
        setCountry(post.country || '');
        setAutoClose(post.auto_close_enabled || false);
        
        if (post.expiry_date) {
            // Format datetime for datetime-local input
            const date = new Date(post.expiry_date);
            const formattedDate = new Date(date.getTime() - date.getTimezoneOffset() * 60000).toISOString().slice(0, 16);
            setExpiryDate(formattedDate);
        }

        setHealthcareExpertiseNeeded(post.healthcare_expertise_needed || '');
        setHighLevelIdea(post.high_level_idea || '');
        setDesiredTechnicalExpertise(post.desired_technical_expertise || '');
        setLevelOfCommitment(post.level_of_commitment || '');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      await axios.put(`https://health-ai-platform-backend.onrender.com/api/posts/${postId}`, {
        title,
        ...
        expiry_date: autoClose && expiryDate ? new Date(expiryDate).toISOString() : null,
